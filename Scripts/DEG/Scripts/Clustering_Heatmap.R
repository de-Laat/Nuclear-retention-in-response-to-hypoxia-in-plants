...
# limma <-  read.csv("DEG_AllContrasts_Gitte_Dirk.csv", row.names=1, check.names=FALSE)
limma <-  read.delim("DEG_AllContrasts_Gitte_Dirk.xls", row.names=1, check.names=FALSE)
...
col.clust.sort="logFC.Hyp2_NUC_TOT" # column on which to order the clusters based on their median value.
...
mydata=limma[(limma[,2]<padj&abs(limma[,1])>log)|(limma[,5]<padj&abs(limma[,4])>1)|(limma[,8]<padj&abs(limma[7])>1)|(limma[,11]<padj&abs(limma[10])>1)|(limma[,14]<padj&abs(limma[13])>1)|(limma[,44]<padj&abs(limma[43])>1)|(limma[,47]<padj&abs(limma[46])>1)|(limma[,50]<padj&abs(limma[49])>1)|(limma[,53]<padj&abs(limma[,52])>1),c(1,4,7,10,13,43,46,49,52)]

## Adition of rownames to the selected datasets
rownames(mydata)=unique(as.character(limma[(limma[,2]<padj&abs(limma[,1])>log)|(limma[,5]<padj&abs(limma[,4])>1)|(limma[,8]<padj&abs(limma[7])>1)|(limma[,11]<padj&abs(limma[10])>1)|(limma[,14]<padj&abs(limma[13])>1)|(limma[,44]<padj&abs(limma[43])>1)|(limma[,47]<padj&abs(limma[46])>1)|(limma[,50]<padj&abs(limma[49])>1)|(limma[,53]<padj&abs(limma[,52])>1),c("Symbol.Hyp2_NUC_TOT")]))
## Library loading
library(cluster)
library(e1071)
...
anno_Sly=read.csv(".csv",header=T)
...
m <- useMart(biomart="plants_mart", dataset="Arabidopsis_thaliana_gene", host="plants.ensembl.org")
...
load("./dif_SL_root_070719_k10_GO_IN.txt")
r<-data.frame
i=1
